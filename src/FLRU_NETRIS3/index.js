 ...
 * JSON Object. It can only parse payload from FLRU+NETRIS3 devices. 
 ...
 * SPDX-PackageVersion: 1.0.0-DEV-20231130-3
 *  
*/
 ...
 * var LEVEL_RANGE_START = 0;
 * var LEVEL_RANGE_END = 1000;
 */

/**
 * The starting value of the level (distance) range.
 * @type {number}
 */
var LEVEL_RANGE_START;

/**
 * The ending value of the level (distance) range.
 * @type {number}
 */
var LEVEL_RANGE_END;
 ...
 * @type {string[]}
 */
var CHANNEL_MEASURAND_CONFIGURATION = ["level"];
...
function decodeDataMessage(input) {
    ...
    if ((input.bytes.length - 3 - 2) < 0) {
        output = addWarningMessage(output, "Not enough data to decode channel (level). Payload must has a length of 2 bytes, input data length: " + input.bytes.length);
    }
...
function decodeDeviceIdentification(input) {
    ...
    output.data.deviceInformation.channelConfigurations = Array.from({ length: 1 });
    ...
    // Channel 0 is always there (level)
...
function checkMeasurementRanges(output) {
    if (typeof LEVEL_RANGE_START === 'undefined') {
        output = addErrorMessage(output, "The LEVEL_RANGE_START was not set.");
    }

    if (typeof LEVEL_RANGE_END === 'undefined') {
        output = addErrorMessage(output, "The LEVEL_RANGE_END was not set.");
    }
    if (LEVEL_RANGE_START >= LEVEL_RANGE_END) {
        output = addErrorMessage(output, "The LEVEL_RANGE_START must not be greater or equal to LEVEL_RANGE_END, " + LEVEL_RANGE_START + " >= " + LEVEL_RANGE_END + ". ");
    }
 ...
 * @access protected
 * @param  {Number} levelRangeStart   range start
 * @param  {Number} levelRangeEnd     range end
 */
function setMeasurementRanges(levelRangeStart, levelRangeEnd) {
    LEVEL_RANGE_START = levelRangeStart;
    LEVEL_RANGE_END = levelRangeEnd;
}
...
/**
 * Returns the real physical value of level based on measurement range
 * @param {Number} channelValue  
 * @access private
 */
function getCalculatedLevel(channelValue) {
    return getCalculatedValue(channelValue, LEVEL_RANGE_START, LEVEL_RANGE_END, GENERIC_DATA_CHANNEL_RANGE_START, GENERIC_DATA_CHANNEL_RANGE_END);
}
...
function getRealValueByChannelName(channelName, channelValue) {
    if (channelName == CHANNEL_MEASURAND_CONFIGURATION_DEFAULT_ORDER[0]) {
        return getCalculatedLevel(channelValue);
    }
...
function getRealValueByChannelNumberAndAlarmType(channelNumber, alarmType, channelValue) {
    if (channelNumber == 0) {
        if (alarmType == 3 || alarmType == 4) {
            return getSlopeValue(channelValue, LEVEL_RANGE_START, LEVEL_RANGE_END);
        }
        else {
            return getThresholdValue(channelValue, LEVEL_RANGE_START, LEVEL_RANGE_END);
        }
    }
...
/**
 * Returns the real threshold value of level based on measurement range (measurend)
 * @param {Number} channelValue  
 * @param  {Number} measurementRangeStart   range start
 * @param  {Number} measurementRangeEnd     range end
 * @access private
...
/**
 * Returns the real physical value of level based on measurement range (measurend/minute)
 * @param {Number} channelValue  
 * @param  {Number} measurementRangeStart   range start
 * @param  {Number} measurementRangeEnd     range end
 * @access private
