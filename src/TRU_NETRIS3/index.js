 ...
 * JSON Object. It can only parse payload from TRU+NETRIS3 devices. 
...
function decodeDataMessage(input) {
    ...
    if ((input.bytes.length - 3 - 2) < 0) {
        output = addWarningMessage(output, "Not enough data to decode channel (temperature). Payload must has a length of 2 bytes, input data length: " + input.bytes.length);
    }
...
function decodeDeviceIdentification(input) {
    ...
    // Channel 0 is always there (temperature)
 ...
 */
function setMeasurementRanges(temperatureRangeStart, temperatureRangeEnd) {
