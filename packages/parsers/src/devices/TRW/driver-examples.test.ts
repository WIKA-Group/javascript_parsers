...
import { createTRWUplinkOutputSchema, DownlinkInputSchema } from './schema'
...
describe('trw parser', () => {
  ...
  it.each(downlinkBadCases)(`should return prefixed error for bad downlink: $description`, (example) => {
    ...
    expect(output).toMatchObject({
      errors: [expect.stringContaining('TRW (JS):')],
    })
  })
...
describe('trw parser range lifecycle', () => {
  it('should decode and encode range-dependent TULIP2/TULIP3 values across parser lifetime (0-10 -> 5-15)', () => {
    ...
    adjustMeasuringRange('temperature', { start: 5, end: 15 })
    ...
    const t2ValueAt0to10 = (tulip2DecodeAt0to10 as any).data.measurement.channels.find((c: any) => c.channelName === 'temperature').value
    const t2ValueAt5to15 = (tulip2DecodeAt5to15 as any).data.measurement.channels.find((c: any) => c.channelName === 'temperature').value
    const t3ValueAt0to10 = (tulip3DecodeAt0to10 as any).data.measurements.find((m: any) => m.channelName === 'temperature').value
    const t3ValueAt5to15 = (tulip3DecodeAt5to15 as any).data.measurements.find((m: any) => m.channelName === 'temperature').value
    ...
    adjustMeasuringRange('temperature', { start: 5, end: 5.5 })
