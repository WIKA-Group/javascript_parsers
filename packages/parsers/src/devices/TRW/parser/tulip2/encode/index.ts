...
import type { TRWTULIP2ConfigurationAction, TRWTULIP2GetConfigurationAction, TRWTULIP2ResetBatteryAction } from '../../../schema/tulip2'
import type { TRWTulip2DownlinkInput } from '../constants'
...
import { TRW_COMMANDS, TRW_DEFAULT_BYTE_LIMIT, TRW_DEFAULT_CONFIGURATION_ID, TRW_DOWNLINK_FEATURE_FLAGS } from '../constants'

type DownlinkCommand = number[]
type DownlinkFrame = number[]

function buildMainConfigCommand(
  config: TRWTULIP2ConfigurationAction['mainConfiguration'],
): DownlinkCommand {
  if (!config) {
    return []
  }
  ...
  const bytes: number[] = [TRW_COMMANDS.SET_MAIN_CONFIG]
...
function buildProcessAlarmCommand(
  ...
  const bytes: number[] = [TRW_COMMANDS.SET_PROCESS_ALARM, 0x00]
...
export function TRWTULIP2EncodeHandler(formattedInput: TRWTulip2DownlinkInput): DownlinkOutput
export function TRWTULIP2EncodeHandler(formattedInput: TRWTulip2DownlinkInput, allowMultiple: false): DownlinkOutput
export function TRWTULIP2EncodeHandler(formattedInput: TRWTulip2DownlinkInput, allowMultiple: true): MultipleDownlinkOutput
export function TRWTULIP2EncodeHandler(formattedInput: TRWTulip2DownlinkInput, allowMultiple = false): DownlinkOutput | MultipleDownlinkOutput {
...
function encodeResetToFactory(): DownlinkFrame {
  const command = [TRW_COMMANDS.RESET_FACTORY]
  return buildDownlinkFrame([command], {
    configId: TRW_DEFAULT_CONFIGURATION_ID,
    maxConfigId: TRW_DOWNLINK_FEATURE_FLAGS.maxConfigId,
    byteLimit: Infinity,
  })
}

function encodeResetBatteryIndicator(input: TRWTULIP2ResetBatteryAction): DownlinkFrame {
  const configId = input.configurationId ?? TRW_DEFAULT_CONFIGURATION_ID
  const command = [TRW_COMMANDS.RESET_BATTERY, 0x00]
  return buildDownlinkFrame([command], {
    configId,
    maxConfigId: TRW_DOWNLINK_FEATURE_FLAGS.maxConfigId,
    byteLimit: Infinity,
...
function encodeDownlinkConfiguration(input: TRWTULIP2ConfigurationAction, allowMultiple: false): DownlinkOutput
function encodeDownlinkConfiguration(input: TRWTULIP2ConfigurationAction, allowMultiple: true): MultipleDownlinkOutput
function encodeDownlinkConfiguration(input: TRWTULIP2ConfigurationAction, allowMultiple: boolean): DownlinkOutput | MultipleDownlinkOutput {
  const configId = input.configurationId ?? TRW_DEFAULT_CONFIGURATION_ID

  const byteLimit = input.byteLimit ?? TRW_DEFAULT_BYTE_LIMIT
...
function encodeGetConfiguration(input: TRWTULIP2GetConfigurationAction, allowMultiple: false): DownlinkOutput
function encodeGetConfiguration(input: TRWTULIP2GetConfigurationAction, allowMultiple: true): MultipleDownlinkOutput
function encodeGetConfiguration(input: TRWTULIP2GetConfigurationAction, allowMultiple: boolean): DownlinkOutput | MultipleDownlinkOutput {
  const commands: DownlinkCommand[] = []

  if (input.mainConfiguration) {
    commands.push([TRW_COMMANDS.GET_MAIN_CONFIG])
  }

  if (input.processAlarmConfiguration) {
    commands.push([TRW_COMMANDS.GET_PROCESS_ALARM, 0x00])
  }
  ...
  const configId = ('configurationId' in input ? input.configurationId : undefined) ?? TRW_DEFAULT_CONFIGURATION_ID
