...
import { createTGUUplinkOutputSchema } from './schema'
...
describe('tGU+NETRIS3 parser', () => {
  ...
  const outputSchema = createTGUUplinkOutputSchema()
...
describe('tGU+NETRIS3 parser range lifecycle', () => {
  it('should decode and encode range-dependent TULIP2/TULIP3 values across parser lifetime', () => {
    ...
    const tulip2DecodeAt100 = decodeUplink({
      bytes: [1, 0, 35, 9, 185, 26, 240],
      fPort: 1,
      recvTime: '1992-12-22T17:00:00+01:00',
    } as any)

    const tulip3DecodeAt100 = decodeUplink({
    ...
    adjustMeasuringRange('temperature', { start: 0, end: 160 })

    const tulip2DecodeAt160 = decodeUplink({
      bytes: [1, 0, 35, 9, 185, 26, 240],
      fPort: 1,
      recvTime: '1992-12-22T17:00:00+01:00',
    } as any)

    const tulip3DecodeAt160 = decodeUplink({
      bytes: [16, 1, 6, 46, 151, 14, 21, 47],
      fPort: 1,
      recvTime: '1992-12-22T17:00:00+01:00',
    } as any)

    const tulip3EncodeAt160 = encodeDownlink({
      protocol: 'TULIP3',
      input: {
        action: 'writeRegisters',
        input: {
    ...
    expect(tulip2DecodeAt160).not.toEqual(tulip2DecodeAt100)
    expect(tulip3DecodeAt160).not.toEqual(tulip3DecodeAt100)
    expect(tulip3EncodeAt100).toMatchObject({ fPort: 1, bytes: expect.any(Array) })
    expect(tulip3EncodeAt160).toMatchObject({ fPort: 1, bytes: expect.any(Array) })
    expect((tulip3EncodeAt160 as any).bytes).not.toEqual((tulip3EncodeAt100 as any).bytes)
  })
})
